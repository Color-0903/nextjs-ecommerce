...
import { CreateOtpDto, FilterOtpDto } from "@/apis/client-axios";
...
export const OtpConfirm = (props: OtpConfirmInterface) => {
    const { identifier, onFinish, submitCode, setSubmitCode } = props;
    ...
    const { mutate, isPending } = useMutation({
        ...
        onError: (error: any) => {
            return <Nofification type='toast-danger' content='Mã OTP không chính xác hoặc đã có lỗi xảy ra!' />
        },
    })
    ...
    return <App>
        <Spinner isLoading={isPending}>
            {
                submitCode && <div id="popup-modal" className="flex bg-white overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
