...
import { CreateOrderDto } from "@/apis/client-axios";
...
export const LisCart = (params: CartInterface) => {
    ...
    const { mutate: Order, isPending: isOrder } = useMutation({
        mutationFn: (dto: CreateOrderDto) => orderApi.orderControllerCreate(dto),
        onSuccess: (data: any) => {
            Notification('Đặt hàng thành công!', 'success');
        },
        onError: (error: any) => {
            Notification('Tài khoản hoặc mật khẩu không chính xác!', 'error');
        },
    })
    ...
    return <Spinner isLoading={isOrder}>
        <div hidden={!isShowCart}>
                ...
                <div className="fixed top-0 p-4 w-full max-w-5xl h-screen mx-auto inset-x-0">
                    <div className="flex flex-col h-screen bg-white rounded-lg dark:bg-gray-700 shadow-2xl pb-4">
                        ...
                        <div className="w-full max-w-7xl px-6 md:px-5 lg-6 mx-auto mt-5 pb-5 overflow-y-auto overflow-x-hidden flex-1">
                            {
                                cart.map((item: any) =>
                                    <div key={item?.source} className="relative rounded-3xl border-2 border-gray-200 p-4  grid-cols-12 mb-1 md:mb-2 max-lg:max-w-lg max-lg:mx-auto gap-y-4 ">
